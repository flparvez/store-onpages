...
const OrderPage = ({params}) => {
    ...
    const {
      ...
      formState: { errors },
        ...
        if (data?.order) {
          setValue("name", data.order.name);
          setValue("email", data.order.email);
          setValue("phone", data.order.phone);
          setValue("address", data.order.address);
          setValue("city", data.order.city);
          setValue("status", data.order.status);
          setValue("transaction", data.order.transaction);
          setValue("ordertrack", data.order.ordertrack);
...
const onSubmit = async (data) => {
  await editOrder({ id: params.id, updatedOrder: data });
  if (!order) {
    <h2>Loading...</h2>
  }
  if (!isLoading) {
